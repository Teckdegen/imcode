...
serve(async (req) => {
  ...
  try {
    // Parse request body
    const { message, context, files = [], apiKey, provider } = await req.json();
...
REMEMBER: Generate EXTENSIVE, COMPREHENSIVE, DEPLOYMENT-READY code. Each file should be substantial with complete implementations, not minimal examples. NO EMPTY FILES EVER.${referencedFiles}`;
    ...
    switch (provider.toLowerCase()) {
      ...
      case 'gemini':
        ...
        break;
      default:
        throw new Error('Unsupported provider. Supported: openai, groq, grok, claude, gemini');
    }
    ...
    switch (provider.toLowerCase()) {
      ...
      case 'grok':
        ...
        break;
      case 'claude':
        aiResponse = data.content?.[0]?.text || data?.content || '';
        break;
      case 'gemini':
    ...
    return new Response(JSON.stringify({ 
      ...
      details: 'Please check if your API key and provider are properly set.'
    }), {
      status: 500,
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    });
